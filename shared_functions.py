...
def convert_date_to_epoch(s) -> int:
    """Parse date from config file with format %Y-%m-%dT%H:%M and return it as a UNIX
    epoch in seconds.
...
def parse_timestamp_argument(arg: str) -> int:
    """Parse a timestamp argument which can either be given as a UNIX epoch in seconds
    or milliseconds, or as a date with format %Y-%m-%dT%H:%M and return it as a UNIX
    epoch in seconds.
